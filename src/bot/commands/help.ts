...
const HELP_TEXT =
  `${E.pin} <b>ПОМОЩЬ</b>\n\n` +
  `${E.shield} Здесь всё необходимое для старта работы с ботом.\n\n` +
  ...
  `${E.moneybag} Пополни баланс — бот получит уведомление автоматически\n\n` +
  `4️⃣ <b>Получай сигналы</b>\n` +
  `${E.lightning} После подтверждения депозита раздел «Сигналы» разблокируется` +
  `</blockquote>\n\n` +
  `${E.starStruck} <b>ЧАСТЫЕ ВОПРОСЫ</b>\n` +
  `<blockquote expandable>` +
  `${E.shield} <b>Зачем боту нужен новый аккаунт?</b>\n` +
  ...
  `${E.chart} <b>Почему сигналы недоступны?</b>\n` +
  ...
  `${E.pen} <b>Мой ID не подтверждается</b>\n` +
  ...
  `${E.lightning} <b>Как работают сигналы?</b>\n` +
